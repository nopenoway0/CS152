...
class EwokParsers extends RegexParsers {
	...
	//def funcall: Parser[Expression] = identifier~"("~opt(operands)~")" ^^{
		...
		case id~"("~Some(exp)~")"=>{
			// add check for identifier
			//if(id.isInstanceOf[Funcall]) throw UndefinedException("YIGHT")
			//Funcall(id.asInstanceOf[Identifier], exp) // proble could be here
			Funcall(id, exp)
		}
		case unknown => throw new SyntaxException("Error")
	}
...
class SithParsers extends WookieParsers{
	...
	def deref: Parser[Expression] = "["~>expression~"]"~opt(parameters) ^^{
		case exp1~"]"~None => Funcall(Identifier("content"), List(exp1))
		case exp2~"]"~Some(args) => throw SyntaxException("Error")
		case _ => throw SyntaxException("Trash")
	}
}